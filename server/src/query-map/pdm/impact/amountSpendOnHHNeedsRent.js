...
export default async (
  ...
  const [results] = await database.raw(
    ...
    SELECT
        ...
        SUM(IF(\`s3_use_of_cash_assistanceq3_5_6_rent\` = 'almost_nothing', 1, 0)) AS \`almost_nothing\`,
        SUM(IF(\`s3_use_of_cash_assistanceq3_5_6_rent\` = 'less_than_half', 1, 0)) AS \`less_than_half\`,
        SUM(IF(\`s3_use_of_cash_assistanceq3_5_6_rent\` = 'more_than_half', 1, 0)) AS \`more_than_half\`,
        SUM(IF(\`s3_use_of_cash_assistanceq3_5_6_rent\` = 'almost_all', 1, 0)) AS \`almost_all\`,
        SUM(IF(\`s3_use_of_cash_assistanceq3_5_6_rent\` = '', 1, 0)) AS \`N/A\`
    FROM
        pdm
    WHERE
