export default async (
  ...
  const [results] = await database.raw(
    ...
    SELECT
        ...
        SUM(IF(\`s3_use_of_cash_assistanceq3_5_1_food\` = 'almost_nothing', 1, 0)) AS \`almost_nothing' \`,
        SUM(IF(\`s3_use_of_cash_assistanceq3_5_1_food\` = 'less_than_half', 1, 0)) AS \`less_than_half'\`,
        SUM(IF(\`s3_use_of_cash_assistanceq3_5_1_food\` = 'more_than_half', 1, 0)) AS \`more_than_half\`,
        SUM(IF(\`s3_use_of_cash_assistanceq3_5_1_food\` = 'almost_all', 1, 0)) AS \`almost_all'\`,
        SUM(IF(\`s3_use_of_cash_assistanceq3_5_1_food\` = '', 1, 0)) AS \`N/A\`
    FROM
        pdm
    WHERE
