export default async (
  ...
  const [results] = await database.raw(
    ...
    SELECT
        ...
        SUM(IF(\`s3_use_of_cash_assistanceq3_5_3_hygiene_items_nfis\` = 'almost_nothing', 1, 0)) AS \`almost_nothing' \`,
        SUM(IF(\`s3_use_of_cash_assistanceq3_5_3_hygiene_items_nfis\` = 'less_than_half', 1, 0)) AS \`less_than_half'\`,
        SUM(IF(\`s3_use_of_cash_assistanceq3_5_3_hygiene_items_nfis\` = 'more_than_half', 1, 0)) AS \`more_than_half\`,
        SUM(IF(\`s3_use_of_cash_assistanceq3_5_3_hygiene_items_nfis\` = 'almost_all', 1, 0)) AS \`almost_all'\`,
        SUM(IF(\`s3_use_of_cash_assistanceq3_5_3_hygiene_items_nfis\` = '', 1, 0)) AS \`N/A\`
    FROM
        pdm
    WHERE
